...
from .util import LearningHandler, make_layer_list, train_gen, get_scale, get_y_preds, run_and_save_embedding, print_accuracy, get_cluster_sols
...
class SiameseNet:
    ...
    def train(self, pairs_train, dist_train, pairs_val, dist_val,
            lr, drop, patience, num_epochs, batch_size, callbacks=None):
        ...
        # train the network
        if callbacks:
            callbacks.append(self.lh)
        else:
            callbacks = [self.lh]

        hist = self.net.fit_generator(train_gen_, epochs=num_epochs, validation_data=validation_data,
...
class SpectralNet:
    def __init__(self, inputs, arch, spec_reg, y_true, y_train_labeled_onehot, y_all, x_all,
    ...
    def train(self, x_train_unlabeled, x_train_labeled, x_val_unlabeled,
            lr, drop, patience, num_epochs, params):
        # create handler for early stopping and learning rate scheduling
        self.lh = LearningHandler(
                lr=lr,
                drop=drop,
        ...
        for i in range(num_epochs):
            ...
            y_valid_ind = np.where(np.isnan(self.y) == False)[0]
            y_valid = self.y[y_valid_ind]
            ...
            acc_array[i] = cur_acc
            run_and_save_embedding(i, cur_x_spectralnet[y_valid_ind], y_valid, acc_array, params, losses,
                                   val_losses)

            self.net.save_weights(os.path.join(params['results_path'], 'SpectralNetWeights.h5'))
            print("finished epoch " + str(i))

        return losses[:i], val_losses[:i], acc_array
